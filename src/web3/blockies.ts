import type { IconOptions } from "../types/main";
...
(() => {
	...
	function createCanvas(imageData: number[], color: string, scale: number, bgcolor: string, spotcolor: string): HTMLCanvasElement {
		const c = document.createElement("canvas");
		const width = Math.sqrt(imageData.length);
		c.width = c.height = width * scale;
		const cc = c.getContext("2d");
		if (!cc) throw new Error("Could not get 2D context from canvas.");
